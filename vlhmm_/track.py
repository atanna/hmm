...
class Track():
    ...
    def _push_peak(self, state, start_pos, end_pos, f, max_score=1000, peak_name="", color0=(0, 255, 0), color1=(255, 0, 0)):
        if state:
            color = color1
        else:
            color = color0

        f.write("chr{chr_i} {chromStart} {chromEnd} {name} {score} + {chromStart} {chromEnd} {rgb[0]},{rgb[1]},{rgb[2]}\n"
            ...
            for i, st in enumerate(states):
                ...
                for j, pos in enumerate(self.sample_pos[i]):
                    if st[j] == curr_st:
                        ...
                        continue
                    self._push_peak(curr_st, start_pos, end_pos, f, peak_name="{}_{}".format(i, j))
                    start_pos = pos
                    end_pos = start_pos + self.bin_size - 1
                    curr_st = st[j]
                self._push_peak(curr_st, start_pos, end_pos, f, peak_name="{}_{}".format(i, j))
    ...
    def create_track_data(self, xmax, priority=19):
        ...
        with open("{}data.bed".format(self.path), "wt") as f:
            ...
            for i, data in enumerate(self.arr_data):
                for j, pos in enumerate(self.sample_pos[i]):
                    f.write("chr{chr_i} {chromStart} {chromEnd} {name} {score}\n"
                            .format(chr_i=self.chr_i,
                                    chromStart=pos,
                                    chromEnd=pos+self.bin_size,
                                    name="{}_{}".format(i, j),
                                    score=int(data[j]*self.max_score / xmax)))
        ...
        with open("{}data_bar.txt".format(self.path), "wt") as f:
            ...
            for i, data in enumerate(self.arr_data):
                f.write("fixedStep chrom=chr{} start={} step={} span={}\n"
                        .format(self.chr_i, self.sample_pos[i][0], self.bin_size, self.bin_size-1))
                for x in data:
                    f.write("{}\n".format(int(x)))
