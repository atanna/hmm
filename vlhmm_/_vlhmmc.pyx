...
def _log_forward(contexts, log_a, log_b, log_context_p, tr_trie, id_c, state_c, log_alpha):
        log_alpha[:] = -np.inf

        n_contexts = len(log_context_p)
        T = len(log_b)
...
def _log_backward(contexts, log_a, log_b, tr_trie, id_c, state_c, log_beta):
    log_beta[:] = np.log(0.)
    log_beta[-1] = np.log(1.)
    T = len(log_b)
...
def _log_ksi(contexts, log_a, log_b, tr_trie, id_c, state_c, log_alpha, log_beta, log_ksi):
    log_ksi[:] = np.log(0.)
    T = len(log_b)
    n, n_contexts = log_a.shape
    for t in range(T - 1):
